...
(function () {
    this.QuizPlugin = function () {
        ...
        if (arguments[0] && typeof arguments[0] === "object") {
            this.options = extendDeafaults(defaults, arguments[0]);
        }
        this.init();
    };

    function extendDeafaults(source, properties) {
        let property;
    ...
    QuizPlugin.prototype.renderQuestions = function (index) {
        ...
        <div class="quiz-questions-item">
            ...
            <ul class="quiz-questions-item__answer">${renderAnswers()}</ul>
        </div>
    `;
    };
    ...
    QuizPlugin.prototype.addListeners = function () {
        ...
        this.options.selector.quiz.addEventListener("click", (event) => {
            if (event.target.classList.contains('btn-next')) {
                const nextQuestionIndex = Number(this.options.selector.questions.dataset.currentStep) + 1;
